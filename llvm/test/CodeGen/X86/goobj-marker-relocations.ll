...
@llvm.compiler.used = appending global [6 x ptr] [ptr @source, ptr @target, ptr @init_source, ptr @init_target, ptr @"type:int", ptr @"go:track.pkg.T.X"], section "llvm.metadata"

define goabiinternal void @source() {
  ret void
}

!goobj.marker_relocs = !{!0, !1, !2, !3, !4}
...
!4 = !{ptr @source, ptr @"go:track.pkg.T.X", i32 21, i64 0}
...
; CHECK-DAG: nonpkgref {{[0-9]+}}: go:track.pkg.T.X abi=0 type=0 size=0
...
; CHECK: reloc {{[0-9]+}}.{{[0-9]+}}: off=0 size=0 type=21 add=0 target=go:track.pkg.T.X
