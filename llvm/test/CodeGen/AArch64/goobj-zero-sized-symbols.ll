; RUN: llc -mtriple=aarch64-apple-darwin-goobj -goobj-package-path=example/pkg -filetype=obj < %s -o %t.arm64.o
...
; CHECK-DAG: symdef [[EMPTY:[0-9]+]]: goallc.{{[0-9a-f]+}}.stmp_{{[0-9]+}} abi=65535 type=3 size=0 align=4 flag=2 flag2=0
; CHECK-DAG: reloc {{[0-9]+}}.{{[0-9]+}}: off=0 size=8 type=1 add=0 target=goallc.{{[0-9a-f]+}}.stmp_{{[0-9]+}}
