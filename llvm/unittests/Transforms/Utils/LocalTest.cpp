...
// These tests use ordinary C IR: storage-to-value debug conversion must not
// depend on Go calling conventions, object formats, or SelectionDAG.
struct AllocaDebugSalvageTest : ::testing::Test {
  LLVMContext C;
  std::unique_ptr<Module> M;
  Function *F = nullptr;

  void build(StringRef Body) {
    std::string IR = R"(
      declare void @use(ptr)
      declare void @initialize(ptr)
      define void @f(i64 %input) !dbg !8 {
      entry:
    )";
    IR += Body;
    IR += R"(
      ...
      !3 = !{i32 2, !"Dwarf Version", i32 4}
      !4 = !{i32 2, !"Debug Info Version", i32 3}
      !8 = distinct !DISubprogram(name: "f", scope: !1, file: !1, line: 1, type: !9, spFlags: DISPFlagDefinition | DISPFlagOptimized, unit: !0)
      !9 = !DISubroutineType(types: !2)
      !11 = !DILocalVariable(name: "scalar", scope: !8, file: !1, line: 2, type: !12)
      !12 = !DIBasicType(name: "long", size: 64, encoding: DW_ATE_signed)
      !13 = !DILocation(line: 2, column: 1, scope: !8)
      !14 = !DILocalVariable(name: "snapshot", scope: !8, file: !1, line: 2, type: !12)
      !15 = !DILocalVariable(name: "pair", scope: !8, file: !1, line: 2, type: !16)
      !16 = !DICompositeType(tag: DW_TAG_structure_type, name: "pair", size: 128, elements: !2)
    )";
    M = parseIR(C, IR.c_str());
    ASSERT_TRUE(M);
    F = M->getFunction("f");
    ASSERT_TRUE(F);
  }

  Instruction *named(StringRef Name) {
    for (BasicBlock &BB : *F)
  ...
  void salvage() {
    SmallVector<Instruction *> Before;
    for (BasicBlock &BB : *F)
      for (Instruction &I : BB)
        Before.push_back(&I);
    auto *AI = cast<AllocaInst>(named("home"));
    salvageDebugInfoForAllocas({AI});
    SmallVector<Instruction *> After;
    for (BasicBlock &BB : *F)
      for (Instruction &I : BB) {
        After.push_back(&I);
        for (DbgVariableRecord &DVR : filterDbgVars(I.getDbgRecordRange())) {
          EXPECT_TRUE(DVR.isDbgValue());
          for (Value *V : DVR.location_ops())
            EXPECT_NE(V, AI);
        }
      }
    EXPECT_EQ(Before, After); // Debug salvage must not change executable IR.
    EXPECT_FALSE(verifyModule(*M, &errs()));
  }
};

TEST_F(AllocaDebugSalvageTest, ScalarDeclarationAndSnapshot) {
  build(R"(
    %home = alloca i64
    #dbg_declare(ptr %home, !11, !DIExpression(), !13)
    store i64 7, ptr %home
    #dbg_value(ptr %home, !14, !DIExpression(DW_OP_deref), !13)
    call void @use(ptr %home)
    ret void
  )");
  salvage();
  unsigned KnownValues = 0;
  for (Instruction &I : F->getEntryBlock())
    for (DbgVariableRecord &DVR : filterDbgVars(I.getDbgRecordRange()))
      if (auto *V = dyn_cast<ConstantInt>(DVR.getVariableLocationOp(0))) {
        EXPECT_EQ(V->getZExtValue(), 7u);
        EXPECT_TRUE(isa<CallInst>(I));
        EXPECT_TRUE(DVR.getExpression()->getElements().empty());
        ++KnownValues;
      }
  EXPECT_EQ(KnownValues, 2u);
}

TEST_F(AllocaDebugSalvageTest, LoadedResult) {
  build(R"(
    %home = alloca i64
    #dbg_declare(ptr %home, !11, !DIExpression(), !13)
    call void @initialize(ptr %home)
    %loaded = load i64, ptr %home
    ret void
  )");
  salvage();
  auto *Ret = F->getEntryBlock().getTerminator();
  auto Records = filterDbgVars(Ret->getDbgRecordRange());
  ASSERT_EQ(std::distance(Records.begin(), Records.end()), 1);
  DbgVariableRecord &Record = *Records.begin();
  EXPECT_EQ(Record.getVariableLocationOp(0), named("loaded"));
  EXPECT_TRUE(Record.getExpression()->getElements().empty());
}

TEST_F(AllocaDebugSalvageTest, AggregateFragmentsAndPartialOverwrite) {
  build(R"(
    %home = alloca { i64, i64 }
    #dbg_declare(ptr %home, !15, !DIExpression(), !13)
    %first = insertvalue { i64, i64 } poison, i64 %input, 0
    %both = insertvalue { i64, i64 } %first, i64 17, 1
    store { i64, i64 } %both, ptr %home
    %part = getelementptr i8, ptr %home, i64 4
    store i32 9, ptr %part
    call void @use(ptr %home)
    ret void
  )");
  salvage();
  bool FoundInput = false, FoundConstant = false, FoundKill = false;
  for (Instruction &I : F->getEntryBlock()) {
    for (DbgVariableRecord &DVR : filterDbgVars(I.getDbgRecordRange())) {
      auto Fragment = DVR.getExpression()->getFragmentInfo();
      if (!Fragment)
        continue;
      Value *V = DVR.getVariableLocationOp(0);
      if (V == F->getArg(0)) {
        EXPECT_EQ(Fragment->OffsetInBits, 0u);
        EXPECT_EQ(Fragment->SizeInBits, 64u);
        FoundInput = true;
      } else if (auto *CI = dyn_cast<ConstantInt>(V);
                 CI && CI->getZExtValue() == 17) {
        EXPECT_EQ(Fragment->OffsetInBits, 64u);
        EXPECT_EQ(Fragment->SizeInBits, 64u);
        FoundConstant = true;
      } else if (isa<PoisonValue>(V) && Fragment->OffsetInBits == 32) {
        EXPECT_EQ(Fragment->SizeInBits, 32u);
        EXPECT_TRUE(isa<CallInst>(I));
        FoundKill = true;
      }
    }
...
TEST_F(AllocaDebugSalvageTest, UnsupportedAddressIsUnavailable) {
  build(R"(
    %home = alloca i64
    store i64 7, ptr %home
    #dbg_value(ptr %home, !11, !DIExpression(), !13)
    call void @use(ptr %home)
    ret void
  )");
  salvage();
  ...
  for (Instruction &I : F->getEntryBlock())
    for (DbgVariableRecord &DVR : filterDbgVars(I.getDbgRecordRange())) {
      EXPECT_TRUE(isa<PoisonValue>(DVR.getVariableLocationOp(0)));
      ++Records;
    }
